...
#   enter     - Open repository in new tmux session (tmux only)
#   alt-enter - Open repository in new tmux window (tmux only)
...
EOF
	...
	if [[ -n "${TMUX:-}" ]]; then
		...
		_fzf_options+=(--bind "enter:execute-silent($git_tmux_cmd new-session $git_repo_name -c '{1}')+abort")
		_fzf_options+=(--bind "alt-enter:execute-silent($git_tmux_cmd new-window $git_repo_name -c '{1}')+abort")
	fi
