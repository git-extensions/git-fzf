...
#
# DESCRIPTION:
#   Uses fd to scan 3 levels deep under GIT_FZF_REPO_PATH (default: ~/Projects).
#   Emits one absolute path per line. No header, no colors — pure data.
#
# ENVIRONMENT:
#   GIT_FZF_REPO_PATH - Root directory to scan (default: ~/Projects)
#
# RETURNS:
#   One absolute path per line. Empty output if no repos found.
#
_git_repo_cmd_list() {
	local dir="${GIT_FZF_REPO_PATH:-$HOME/Projects}"
	dir="${dir/#\~/$HOME}"
